...
/**
 * Schools directory (§4.4): workspace table (§1.4-4) — live search, filters,
 * export, priority gauges, chips, fit-sorted by default.
 */
  ...
  const rows = useMemo(
    () =>
      [...SCHOOLS]
        ...
        .filter((s) => s.name.toLowerCase().includes(query.toLowerCase()) || s.region.toLowerCase().includes(query.toLowerCase()))
    ...
    >
      <div className="p-6">
        <div className="mb-5">
          ...
          <p className="mt-0.5 text-sm text-muted">342 identified across Zimbabwe · 12 tracked</p>
        </div>
        ...
        <Card className="!p-0">
          ...
          {rows.map((s) => (
            ...
            >
              ...
              <div className="hidden text-right text-sm tabular-nums text-muted lg:col-span-1 lg:block">{s.enrol.toLocaleString()}</div>
              <div className="lg:col-span-2 lg:pl-6"><Chip tone={productTone(s.product)}>{s.product}</Chip></div>
              <div className="hidden lg:col-span-1 lg:block"><Chip tone={engineTone(s.eng)} dot>{s.eng}</Chip></div>
