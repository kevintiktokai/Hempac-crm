...
/**
 * Lead Engine (§4.2): dark identity band with pulse + targeting chips +
 * cap + on/off, funnel strip, ranked table with fit + "why this school",
 * engine-settings side panel.
 */
...
export default function EnginePage() {
  const rows = SCHOOLS.filter((s) => s.eng !== "In pipeline").sort((a, b) => b.fit - a.fit);
  return (
    <PageState
      skeleton="table"
      empty={
        ...
        <div className="overflow-hidden rounded-hero border border-line bg-card shadow-card">
          <div className="flex flex-wrap items-center justify-between gap-3 bg-ink px-5 py-4">
              ...
              <div>
                <div className="flex items-center gap-2 text-sm font-semibold text-white">
                  ...
                  <span className="inline-flex items-center gap-1 rounded-full bg-success/25 px-2 py-0.5 text-[10px] font-semibold text-[#9BE0A6]">
                    <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-[#7FE08C]" /> Active
                  </span>
                </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-full bg-white/10 px-2.5 py-1 text-[11px] font-medium text-[#CFE0D4]">Private · Trust · Mission</span>
              <span className="rounded-full bg-white/10 px-2.5 py-1 text-[11px] font-medium text-[#CFE0D4]">Harare +3 regions</span>
              <span className="rounded-full bg-terra/25 px-2.5 py-1 text-[11px] font-medium text-[#E7B79E]">Cap: 10 / day</span>
        ...
        <div className="grid gap-5 xl:grid-cols-[1fr_300px]">
          <Card className="min-w-0 !p-0">
            ...
            {rows.map((s) => (
              ...
              >
                ...
                <div className="md:col-span-2"><FitBar v={s.fit} /></div>
                ...
                <div className="justify-self-end md:col-span-2 md:justify-self-start">
                  <Chip tone={engineTone(s.eng)}>
                    <MessageCircle size={11} /> {s.eng}
                  </Chip>
                </div>
              </Link>
            ))}
          ...
          <Card className="h-fit">
            <div className="mb-4 flex items-center justify-between">
              ...
              <Switch defaultChecked aria-label="Engine on/off" />
            </div>
            {([
              ...
              <div key={label} className="mb-4">
                <div className="mb-1.5 text-[11px] font-semibold uppercase tracking-wide text-faint">{label}</div>
                <div className="flex flex-wrap gap-1.5">
                  {values.map((v) => <Chip key={v} tone="green">{v}</Chip>)}
                </div>
              </div>
            ))}
            <div className="mb-4">
              <div className="mb-1.5 text-[11px] font-semibold uppercase tracking-wide text-faint">Product focus</div>
              <Chip tone="terra">Smart Boards</Chip>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-semibold tabular-nums text-body">10 / day</span>
            </div>
