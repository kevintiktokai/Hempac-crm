...
## Phase B — Sprint 4 (suggestion engine)

| # | Decision | Rationale |
|---|----------|-----------|
| 52 | **LLM provider: OpenAI** (client decision, Jul 2026) — activated by `OPENAI_API_KEY` on the Convex deployment (`npx convex env set`), model via `OPENAI_MODEL` (default gpt-4o-mini), JSON-mode chat completions called directly from Convex actions (no SDK dependency). Until the key is set, a small labelled heuristic covers obvious conversational cues (payment/PO signals → stage change; pricing requests → task) and the time sweep uses templated wording. | "We're gonna use OpenAI, go ahead and build and I'll plug it in later." |
| 53 | Conversation trigger: the sync schedules analysis only for threads that received new inbound messages that sweep (≤3/sweep); proposals cap at 2/analysis and dedupe as one pending suggestion per type per school. Time trigger: daily 04:30 UTC sweep — Awaiting Term fires within 28 days of the next Zimbabwe term start (≈13 Jan / 5 May / 8 Sep), No Response nudges immediately, Awaiting Funds checks in after 14 days. | Human-paced budget (addendum §10) and no suggestion pile-up; term calendar to confirm with the client. |
| 54 | Dedupe is against **pending** suggestions only — dismissing one lets the next sweep raise it again. The engine only ever inserts pending suggestions; every effect still applies exclusively through acceptSuggestion (Gate 2). | Re-raising after dismissal is the accountability feature ("tomorrow comes back"); if it annoys in practice, add a cooldown per deal. |
